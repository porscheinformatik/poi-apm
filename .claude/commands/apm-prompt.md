---
argument-hint: Optional target prompt and/or instructions; defaults to reusing the
  current chat.
arguments:
- target_prompt
- task_instructions
...
2. Determine intent and source:
   - If `$source_mode` is `instructions`, use `$task_instructions` as the source.
   - If `$source_mode` is `chat` or omitted, default to making the current chat reusable as a prompt.
   - If both chat context and `$task_instructions` are provided, ask which source should be primary.
...
4. Resolve the target prompt path:
   - prefer `$target_prompt` when provided
...
9. Validate structure and quality:
   ...
   - input placeholders use `$name` consistently
